...
class CrossEntropyLoss_Casual_ID(BaseLoss):
    ...
    def forward(self, logits, labels):
        ...
        for i in range(logits.shape[0]):
            logits_ = logits[i,:,:,:]
            logits_ = logits_.permute(1, 0, 2).contiguous()
            p, _, c = logits_.size()
            log_preds = F.log_softmax(logits_ * self.scale, dim=-1)
            one_hot_labels = self.label2one_hot(
                labels, c).unsqueeze(0).repeat(p, 1, 1)
            loss = self.compute_loss(log_preds, one_hot_labels)
            loss_all += loss
        self.info.update({'casual_softmax_id': loss_all.detach().clone()})

        return loss_all, self.info
