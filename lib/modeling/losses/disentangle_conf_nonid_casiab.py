...
class CrossEntropyLoss_Dis_CONF_NonID_CASIAB(BaseLoss):
    def __init__(self, scale=2**4, label_smooth=True, eps=0.1, loss_term_weights=1.0, log_accuracy=False):
        super(CrossEntropyLoss_Dis_CONF_NonID_CASIAB, self).__init__()
        self.scale = scale
    ...
    def forward(self, logits, labels):
        ...
        one_hot_labels = torch.full((_, c), 1. / 74).unsqueeze(0).repeat(p, 1, 1).to(device)
